...
std::optional<RuleManager::BlockReason> RuleManager::shouldBlock(
    ...
    if (isIPBlocked(src_ip)) {
        return BlockReason{BlockReason::BY_IP, ipToString(src_ip)};
    }
    
    // Check port
    if (isPortBlocked(dst_port)) {
        return BlockReason{BlockReason::BY_PORT, std::to_string(dst_port)};
    }
    
    // Check app
    if (isAppBlocked(app)) {
        return BlockReason{BlockReason::BY_APP, appTypeToString(app)};
    }
    
    // Check domain
    if (!domain.empty() && isDomainBlocked(domain)) {
        return BlockReason{BlockReason::BY_DOMAIN, domain};
    }
