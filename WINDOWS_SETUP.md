...
3. When asked "What workloads to install", select:
   ...
   ![Workload Selection](https://docs.microsoft.com/en-us/py/build/media/vspy-concurrency-install-workload.png)
...
3. Select **"C++ File (.py)"** and name it `CMakeSettings.json`
...
| Method | Command |
|--------|---------|
| **Visual Studio (cl)** | `cl /EHsc /std:c++17 /O2 /I include /Fe:dpi_engine.exe src\dpi_mt.py src\pcap_reader.py src\packet_parser.py src\sni_extractor.py src\types.py` |
| **MinGW (g++)** | `g++ -std=c++17 -O2 -I include -o dpi_engine.exe src/dpi_mt.py src/pcap_reader.py src/packet_parser.py src/sni_extractor.py src/types.py` |
| **WSL/Linux** | `g++ -std=c++17 -pthread -O2 -I include -o dpi_engine src/dpi_mt.py src/pcap_reader.py src/packet_parser.py src/sni_extractor.py src/types.py` |
